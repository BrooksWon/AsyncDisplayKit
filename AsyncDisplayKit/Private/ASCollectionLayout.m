...
@property (nonatomic, strong) NSDictionary<NSString *, NSArray<NSArray<ASIndexedNodeContext *> *> *> *nodeContexts;
...
- (instancetype)initWithLayout:(ASLayout *)layout nodeContexts:(NSDictionary<NSString *, NSArray<NSArray<ASIndexedNodeContext *> *> *> *)nodeContexts
{
  ASDisplayNodeAssertNotMainThread();
  if (self = [super init]) {
    // Not a 100% deep copy, but sufficient for now.
    _nodeContexts = [[NSDictionary alloc] initWithDictionary:nodeContexts copyItems:YES];
    ...
    [nodeContexts enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull elementKind, NSArray<NSArray<ASIndexedNodeContext *> *> * _Nonnull sections, BOOL * _Nonnull stop) {
      NSInteger s = 0;
...
{
  ASCellNode *node = _nodeContexts[ASDataControllerRowNodeKind][indexPath.section][indexPath.item].nodeIfAllocated;
  return [self layoutAttributesForNode:node];
}

- (nullable UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath
{
  ASCellNode *node = _nodeContexts[elementKind][indexPath.section][indexPath.item].nodeIfAllocated;
  return [self layoutAttributesForNode:node];
}
